...
métodos de construção interativa de atributos. A seguir, na
Seção~\ref{sec:rd}, apresenta-se uma discussão sobre os
métodos de redução de dimensionalidade, com um enfoque
especial para métodos interativos. Na Seção~\ref{sec:tr},
apresenta-se um levantamento sobre pesquisas em construção
interativa de atributos, um tema que não conta com uma
...
Esse tipo de representação é útil para se ter uma visão
...
dependências entre os atributos. O principal
mecanismo de interação da ferramenta BD é a seleção que se
...
ambos os métodos, VaR e BD, é não permitir que o
...
\section{Transformação Interativa de
...
por essa ferramenta. Por exemplo, a forma como a amostragem
é realizada. Utilizando uma amostragem aleatória, não
existem garantias de que elementos de cada estrutura
presente nos dados são escolhidos.  Consequentemente, o
usuário precisaria de mais iterações para agregar aos dados
o seu conhecimento.
...
apresentam limitações. Essas limitações assim como as
principais características dos métodos interativos foram
elencadas na Tabela~\ref{tab:tr}, a qual servirá como base
para a descrição da proposta deste trabalho de mestrado.
...
\begin{table}
    ...
    \begin{tabular}{|C{4cm}|C{3cm}|C{3cm}|C{3cm}|}
        ...
        \hline
        \cite{Gladys2013}       & \redc & \redc & \greenc\\ 
        \hline
    \end{tabular}
    \label{tab:tr}
\end{table}
